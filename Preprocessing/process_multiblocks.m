...
ans_runmode = 0;
while ~ans_runmode
	runmode = input('\nStart a (n)ew session or resume the (p)revious session? ','s');
	if strcmp(runmode,'n') | strcmp(runmode,'p')
		ans_runmode = 1;
	else
...
% Assing initial index for global loop based on runmode
if strcmp(runmode,'n')
	clear
	close all
	...
	ans_savetype = 0;
	while ~ans_savetype
		savetype = input('\nPlease choose the saving mode, (q)uick or (f)ull: ','s');
		if strcmp(savetype,'q') | strcmp(savetype,'f')
			ans_savetype = 1;
		else
			fprintf('Wrong answer!')
		end
	...
	t_tic = tic;
	t_run = clock;
	%% Define some global constants, etc. and log them
	setup = containers.Map();
	setup('tankdir') = 'Test1';
	setup('rawdir') = 'Test1_Raw';
	setup('behavdir') = 'BehaviorReplicated';
	setup('excldata') = {'noise' 'tc' 'cellfun'};
	setup('resultdir') = sprintf('Test1_Proc_%i%02i%02i%02i%02i',t_run(1:5));
	setup('df') = [.5 3 5 12];
	setup('audtthresh') = 0.015; % threshold for meaningful auditory stimulus.
	setup('joythresh') = 1000; % threshold for meaningful joystick movements.
	setup('tmwndwth') = 40; % width of time window(in msec) for similarity index.
	setup('tmwndmov') = 40; % size of moving of time window(in msec) for similarity index.
	setup('mindur') = 0; % minimum post-stimulus duration considered as long enough.
	setup('leftoffset') = 116.9; % offset for left-cut(in msec) to include last two tones.
	
	% Assign current setup
	STARTDIR = pwd;
	TANKDIR = setup('tankdir');
	RAWDIR = setup('rawdir');
	BEHAVDIR = setup('behavdir');
	EXCLDATA = setup('excldata');
	RESULTDIR = setup('resultdir');
	DF = setup('df');
	AUDTTHRESH = setup('audtthresh');
	JOYTHRESH = setup('joythresh');
	TMWNDWTH = setup('tmwndwth');
	TMWNDMOV = setup('tmwndmov');
	MINDUR = setup('mindur');
	LEFTOFFSET = setup('leftoffset');
	
	
	%% Set things up for processing multiple blocks.
	% Check if all the data needed are there.
	if ismember(BEHAVDIR,strsplit(ls))
		if ismember(RAWDIR,strsplit(ls))
			fprintf('\nBoth behavioral and neural data have found. Good to go!\n')
			fprintf('\nWarning! Processing multiple blocks may take a while.\n')
			ans_contproc = 0;
			while ~ans_contproc
				contproc = input('\nContinue to process?(y/n): ','s');
				if strcmp(contproc,'y')
					ans_contproc = 1;
					fprintf('\nContinuing...\n')
				elseif strcmp(contproc,'n')
					fprintf('\nProcessing multiblock aborted!\n\n')
					ans_contproc = 1;
					cd(STARTDIR)
					return
				else
			...
			fprintf('\nRaw neural data have not found! Please check out. Thanks!\n')
			fprintf('Multiblock processing aborted!\n\n')
			cd(STARTDIR)
			return
		end
	else
		fprintf('\nBehavioral data have not found! Please check out. Thanks!\n')
		fprintf('Multiblock processing aborted!\n\n')
		cd(STARTDIR)
		return
	end
	% Filter target blocks.
	behavlist = sort(strsplit(ls(BEHAVDIR)));
	neurallist = sort(strsplit(ls(RAWDIR)));
	skipfile = [];
	for i=1:length(behavlist)
		if cellfun('isempty',behavlist(i))
			skipfile(end+1) = i;
		elseif ~ismember(behavlist(i),neurallist)
			skipfile(end+1) = i;
		else
			for j=EXCLDATA
				if ~isempty(findstr(char(j),char(behavlist(i))))
					skipfile(end+1) = i;
					break
				end
			end
	...
	behavlist(skipfile) = [];
	% Just rename.
	targetlist = behavlist;
	num_blocks = length(targetlist);
	num_df = length(DF);
	
	% Prepare log for error
	log_error={};
	
	
	%% Start processing multiple blocks.
	% Define global variables to store results across all blocks.
	si_tot = struct('blockname',{},'si_peak_fw',{},'si_peak_bw',{},'si_itpc_fw',{},'si_itpc_bw',{});
	% Initialize global loop index l.
	initindx = 1;
else
	initindx = l;
end

for l=initindx:length(targetlist)
	LOCSTARTDIR = pwd;
	% Clean up
	if l>1
		clearvars -except l t_tic t_run setup savetype targetlist num_blocks num_df log_error si_tot STARTDIR LOCSTARTDIR f1 f2 f3
	...
	% Assign blockname from list of behavioral files
	blockname = char(strrep(targetlist(l),'.mat',''));
	si_tot(l).blockname = blockname;
	% Go to the directory containing neural data
	cd(RAWDIR)
	% Load neural data.
	fprintf('\n--------------------------------------------------\n')
	fprintf('\nLoading neural data %s...\n',blockname)
	...
	if length(behavdata)==num_tottrials
		...
		fprintf('Sorry!\n\n')
		% Go back to the directory that has raw waveforms.
		msg_numtrial = 'The number of trials between neural and behavior data does NOT match'
		log_error(end+1,1:2) = {blockname,msg_numtrial};
		cd(LOCSTARTDIR)
		continue
	end
	...
	if num_goodtrials
		...
		fprintf('\nSorry. No good trial was found. Block process passed.\n')
		fprintf('Thank you!\n\n');
		cd(LOCSTARTDIR)
		continue
	end
	...
	cd(RESULTDIR)
	% Save the workspace
	if strcmp(savetype,'q')
		fprintf('Saving the result in quick mode...\n')
		PROCFILE = sprintf('%s_quick_%03i-%02i-%03i_%i%02i%02i%02i%02i.mat',blockname,TMWNDWTH,TMWNDMOV,MINDUR,t_run(1:5));
		save(PROCFILE,'setup','behavdec_tr','goodtrials','itpc_*','peak_*','si_*')
	elseif strcmp(savetype,'f')
		fprintf('Saving the result in full mode...\n')
		PROCFILE = sprintf('%s_full_%03i-%02i-%03i_%i%02i%02i%02i%02i.mat',blockname,TMWNDWTH,TMWNDMOV,MINDUR,t_run(1:5));
		save(PROCFILE)
	end
	% Save figures
	fprintf('\nSaving the figures...\n')
	...
	% Store the results for current block to global variables.
	si_tot(l).si_peak_fw = si_peak_fw;
	si_tot(l).si_peak_bw = si_peak_bw;
	si_tot(l).si_itpc_fw = si_itpc_fw;
	si_tot(l).si_itpc_bw = si_itpc_bw;
...
fprintf('\nMultiblock processing successfully completed!\n',blockname)
