function [] = calculate_firing_rate_v3(monkeyName,sessionDate,driveID,binTime_ms,stepTime_ms,Tag)
	...
	hiTime_ms = 5;	% duration of active bins in ms.
	numHiBin = ceil(hiTime_ms/stepTime_ms);	% number of consecutive "hi" bins to define onset (firing rate higher than sigFact*SD of mean firing rate).
	...
	for idx_clust = 1:numClust
		...
		for i = 1:length(diff_bidx_hi)-(numHiBin-1)
			if isequal(diff_bidx_hi(i:i+numHiBin-1),ones(numHiBin,1)) && bidx_hi_clust{idx_clust,2}(i)>bidx_blOff(1)+floor(forbWindow/stepTime_ms)+1 && bidx_hi_clust{idx_clust,2}(i)<bidx_blOff(1)+lim_onset*1000/stepTime_ms
		...
		for i = 1:length(diff_bidx_hi_A)-(numHiBin-1)
			if isequal(diff_bidx_hi_A(i:i+numHiBin-1),ones(numHiBin,1)) && bidx_hi_clust{idx_clust,3}(i)>floor(forbWindow/stepTime_ms)+1 && bidx_hi_clust{idx_clust,3}(i)<lim_onset*1000/stepTime_ms
				% Index of delayed onset
		...
		for i = 1:length(diff_bidx_hi_B)-(numHiBin-1)
			if isequal(diff_bidx_hi_B(i:i+numHiBin-1),ones(numHiBin,1)) && bidx_hi_clust{idx_clust,4}(i)>floor(forbWindow/stepTime_ms)+1 && bidx_hi_clust{idx_clust,4}(i)<lim_onset*1000/stepTime_ms
				% Index of delayed onset
