%function [STRFClass] = classify_strf_params_v4(monkeyName,clustType,p_crit,tag)
	...
	savetag = input('Enter a save tag (without underscore): ','s');
	...
	dir_result = fullfile('~','STRF',sprintf('STRFParams%s',datatag));
	...
	for idx_data = 1:length(list_result)
		%try
			...
			for idx_clust = 1:length(clust2proc)
				for idx_UberSTRF = 1:length(UberSTRF)
					if UberSTRF(idx_UberSTRF).ClustNum == clust2proc(idx_clust)
						...
						BF_par(1,idx_clust) = UberSTRF(idx_UberSTRF).RFParam.BF;
						BFHz_par(1,idx_clust) = UberSTRF(idx_UberSTRF).RFParam.BFHz;
						BW_par(1,idx_clust) = UberSTRF(idx_UberSTRF).RFParam.BW;
						BWHz_par(1,idx_clust) = UberSTRF(idx_UberSTRF).RFParam.BWHz;
						bSMF_par(1,idx_clust) = UberSTRF(idx_UberSTRF).RFParam.bSMF(1);
						...
						FmUpperCutoff_par(1,idx_clust) = UberSTRF(idx_UberSTRF).RFParam.FmUpperCutoff;
			...
			BFHz{idx_data,1} = BFHz_par';
			BW{idx_data,1} = BW_par';
			BWHz{idx_data,1} = BWHz_par';
			bSMF{idx_data,1} = bSMF_par';
			Delay{idx_data,1} = Delay_par';
			DSI{idx_data,1} = DSI_par';
			Duration{idx_data,1} = Duration_par';
			FmUpperCutoff{idx_data,1} = FmUpperCutoff_par';
	...
	STRFClass.FmUpperCutoff = FmUpperCutoff;
	...
	save(sprintf('%s/STRFClass_%s_%s_%s.mat',dir_class,monkeyName,clustTypeName,savetag),'STRFClass')
	
	if ifPlot == 'y'
		plot_strf_params_v4_2
	end
