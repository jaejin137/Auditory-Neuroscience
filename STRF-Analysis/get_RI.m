function get_RI(monkeyName,sessionDate,driveID,clustType)
	...
	clustNum = cell2mat(st_clu(find(contains(st_clu(:,3),clustType)),1));
	...
	for idx_clust = 1:numel(clustNum)
		fprintf('\nProcessing cluster %i, %i out of %i\n',clustNum(idx_clust),s,numel(clustNum))
		...
		try
			...
			% Save the result
			STRFSigData{s,1} = clustNum(idx_clust);
	...
	save(sprintf('RI_%s-%s_%s.mat',monkeyName,sessionDate,driveID))
	plot_RI
