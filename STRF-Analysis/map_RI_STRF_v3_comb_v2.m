...
dir_RISTRF_Fig = fullfile('~','STRF','RISTRF_Figures');
...
if p6_core < .05 && p6_belt < .05
	...
	[h_ks_6, p_ks_6, k_ks_6] = kstest_2s_2d([[SigRISTRF(idx_core).Dur]' [SigRISTRF(idx_core).Delay]'], [[SigRISTRF(idx_belt).Dur]' [SigRISTRF(idx_belt).Delay]']);
	%if p_corrcoeff6 < .05
	if h_ks_6
		sig_r_label6 = '*';
	else
