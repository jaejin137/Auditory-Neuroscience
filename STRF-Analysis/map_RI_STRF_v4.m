...
htbl = nan(15,15,14,2);
htbl_supra = nan(15,15,14,2);
htbl_infra = nan(15,15,14,2);
...
for m = 1:2
	...
	for k = 1:numel(coord_uniq_sig)
		...
		htbl(coord_uniq_uber{k}(2)+8,coord_uniq_uber{k}(1)+8,1,m) = RISTRFMap_Supra(k).numSigClust/RISTRFMap_Supra(k).numAllClust*100;
		htbl(coord_uniq_uber{k}(2)+8,coord_uniq_uber{k}(1)+8,2,m) = log10(RISTRFMap_Supra(k).p_median);
		try
			htbl(coord_uniq_uber{k}(2)+8,coord_uniq_uber{k}(1)+8,3,m) = RISTRFMap_Supra(k).BF;
			htbl(coord_uniq_uber{k}(2)+8,coord_uniq_uber{k}(1)+8,4,m) = RISTRFMap_Supra(k).BFHz;
			htbl(coord_uniq_uber{k}(2)+8,coord_uniq_uber{k}(1)+8,5,m) = RISTRFMap_Supra(k).Delay;
			htbl(coord_uniq_uber{k}(2)+8,coord_uniq_uber{k}(1)+8,6,m) = RISTRFMap_Supra(k).PD;
			htbl(coord_uniq_uber{k}(2)+8,coord_uniq_uber{k}(1)+8,7,m) = RISTRFMap_Supra(k).t10;
			htbl(coord_uniq_uber{k}(2)+8,coord_uniq_uber{k}(1)+8,8,m) = RISTRFMap_Supra(k).t50;
			htbl(coord_uniq_uber{k}(2)+8,coord_uniq_uber{k}(1)+8,9,m) = RISTRFMap_Supra(k).Dur;
			htbl(coord_uniq_uber{k}(2)+8,coord_uniq_uber{k}(1)+8,10,m) = RISTRFMap_Supra(k).PLI;
			htbl(coord_uniq_uber{k}(2)+8,coord_uniq_uber{k}(1)+8,11,m) = RISTRFMap_Supra(k).BW;
			htbl(coord_uniq_uber{k}(2)+8,coord_uniq_uber{k}(1)+8,12,m) = RISTRFMap_Supra(k).BWHz;
			htbl(coord_uniq_uber{k}(2)+8,coord_uniq_uber{k}(1)+8,13,m) = RISTRFMap_Supra(k).bSMF;
			htbl(coord_uniq_uber{k}(2)+8,coord_uniq_uber{k}(1)+8,14,m) = RISTRFMap_Supra(k).FmUpperCutoff;
		end
	...
	saveas(h_corr{m},sprintf('~/STRF/RISTRF_Figures/RISTRF_Corr_v4_%s_%s.png',monkeyName{m},tag))
	%save(fullfile(dir_RISTRF,sprintf('RISTRF_v4_%s_%s.mat',tag,monkeyName{m})))
%end
...
%for m = 1:2
	...
	for ldx_plt = 1:8
		...
		%h_n1 = heatmap((flipud(htbl_supra(:,:,ldx_plt,m))-flipud(htbl_infra(:,:,ldx_plt,m)))./flipud(htbl_infra(:,:,ldx_plt,m)));
		%cmin = min(min((flipud(htbl_supra(:,:,ldx_plt,m))-flipud(htbl_infra(:,:,ldx_plt,m)))./flipud(htbl_infra(:,:,ldx_plt,m))));
		%cmax = max(max((flipud(htbl_supra(:,:,ldx_plt,m))-flipud(htbl_infra(:,:,ldx_plt,m)))./flipud(htbl_infra(:,:,ldx_plt,m))));
		%cmax = max(h_n1.ColorData(find(~isinf(h_n1.ColorData))));
		h_n1 = heatmap(flipud(htbl_supra(:,:,ldx_plt,m))-flipud(htbl_infra(:,:,ldx_plt,m)),'MissingDataLabel','');
		cmin = min(min(flipud(htbl_supra(:,:,ldx_plt,m))-flipud(htbl_infra(:,:,ldx_plt,m))));
		cmax = max(max(flipud(htbl_supra(:,:,ldx_plt,m))-flipud(htbl_infra(:,:,ldx_plt,m))));
		caxis([cmin cmax])
		inc = (cmax-cmin)/256;
	...
	for ldx_plt = 9:14
		...
		%h_n2 = heatmap((flipud(htbl_supra(:,:,ldx_plt,m))-flipud(htbl_infra(:,:,ldx_plt,m)))./flipud(htbl_infra(:,:,ldx_plt,m)));
		%cmin = min(min((flipud(htbl_supra(:,:,ldx_plt,m))-flipud(htbl_infra(:,:,ldx_plt,m)))./flipud(htbl_infra(:,:,ldx_plt,m))));
		%cmax = max(max((flipud(htbl_supra(:,:,ldx_plt,m))-flipud(htbl_infra(:,:,ldx_plt,m)))./flipud(htbl_infra(:,:,ldx_plt,m))));
		%cmax = max(h_n2.ColorData(find(~isinf(h_n2.ColorData))));
		h_n1 = heatmap(flipud(htbl_supra(:,:,ldx_plt,m))-flipud(htbl_infra(:,:,ldx_plt,m)),'MissingDataLabel','');
		cmin = min(min(flipud(htbl_supra(:,:,ldx_plt,m))-flipud(htbl_infra(:,:,ldx_plt,m))));
		cmax = max(max(flipud(htbl_supra(:,:,ldx_plt,m))-flipud(htbl_infra(:,:,ldx_plt,m))));
		caxis([cmin cmax])
	...
	saveas(h_map(m),sprintf('~/STRF/RISTRF_Figures/RISTRF_Map1_v4_%s_%s.png',monkeyName{m},tag))
	saveas(h_map2(m),sprintf('~/STRF/RISTRF_Figures/RISTRF_Map2_v4_%s_%s.png',monkeyName{m},tag))
	saveas(h_map_supra(m),sprintf('~/STRF/RISTRF_Figures/RISTRF_Map1_v4_%s_Supra_%s.png',monkeyName{m},tag))
	saveas(h_map2_supra(m),sprintf('~/STRF/RISTRF_Figures/RISTRF_Map2_v4_%s_Supra_%s.png',monkeyName{m},tag))
	saveas(h_map_infra(m),sprintf('~/STRF/RISTRF_Figures/RISTRF_Map1_v4_%s_Infra_%s.png',monkeyName{m},tag))
	saveas(h_map2_infra(m),sprintf('~/STRF/RISTRF_Figures/RISTRF_Map2_v4_%s_Infra_%s.png',monkeyName{m},tag))
	saveas(h_map_diff(m),sprintf('~/STRF/RISTRF_Figures/RISTRF_Map1_v4_%s_Diff_%s.png',monkeyName{m},tag))
	saveas(h_map2_diff(m),sprintf('~/STRF/RISTRF_Figures/RISTRF_Map2_v4_%s_Diff_%s.png',monkeyName{m},tag))
