function plot_firing_rate_auto_v2(monkeyName,binSize,stepSize,Tag)
	...
	dir_fr = fullfile('~','STRF',sprintf('FR_%i_%i_%s',binSize,stepSize,Tag));
	...
	%for idx_fr = 4
		...
		%try
			...
			if recArea == 'core'
				...
				UberDelay_core(end+1:end+length(idx_clust_fr),1) = latency_clust(idx_clust_fr,6);
				UberRI_core(end+1:end+length(idx_clust_ri),1) = [STRFSig(idx_clust_ri).p]';
			elseif recArea == 'belt'
				UberDelay(idx_fr).AreaIdx = 2;
				UberDelay_belt(end+1:end+length(idx_clust_fr),1) = latency_clust(idx_clust_fr,6);
