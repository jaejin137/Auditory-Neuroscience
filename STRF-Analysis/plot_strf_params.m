...
for idx_data = 1:length(list_result)
	subplot(3,4,1)
	scatter3(Delay{idx_data},Duration{idx_data},BF{idx_data},10,areaColor{strcmp(areaColor(:,1),Area{idx_data}),2},'filled')
	...
	subplot(3,4,5)
	scatter3(PeakDelay{idx_data},PeakEnvDelay{idx_data},PeakBF{idx_data},10,areaColor{strcmp(areaColor(:,1),Area{idx_data}),2},'filled')
	...
	subplot(3,4,9)
	scatter3(PLI{idx_data},PLI2{idx_data},DSI{idx_data},10,areaColor{strcmp(areaColor(:,1),Area{idx_data}),2},'filled')
...
drawnow
suptitle(sprintf('%s, %s',monkeyName,clustTypeName))
...
zlabel('DSI')
drawnow
suptitle(sprintf('%s, %s, k-means clustering (magenta:cluster 1, cyan:cluster 2)',monkeyName,clustTypeName))
...
for l = 1:size(strf_featArr,3)
	...
	figure(h_dist)
	subplot(3,3,l)
	%p1 = histogram(strf_featArr(idx_core,:,l),binEdges,'Normalization','probability','FaceColor','r');
	p1 = histogram(strf_feat_ninf_core,binEdges,'Normalization','probability','FaceColor','r');
	hold on
	%p2 = histogram(strf_featArr(idx_belt,:,l),binEdges,'Normalization','probability','FaceColor','g');
	p2 = histogram(strf_feat_ninf_belt,binEdges,'Normalization','probability','FaceColor','g');
	xlim([binMin binMax])
	title(label_feat{l})
	legend('core','belt')
