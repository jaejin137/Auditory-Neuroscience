...
if strcmp(ModType,'dB')
	...
	while ~feof(fid) & TrigCount<length(Trig)-1
		...
		if TrigCount/NBlocks==round(TrigCount/NBlocks)
			...
			%Displaying STRF1
			pcolor(taxis,log2(faxis/500),fliplr(sum(sum(STRF1,4),3))/sqrt(PP)/double(T)); %Edited Chen Oct. 2008
			shading flat,colormap jet,colorbar
			title(['No = ' int2str(sum(No1)) ' ( Spikes ), Wo = ' num2str(sum(No1)/T,5) ' ( Spikes/Sec )'])

			%Displaying STRF2
			subplot(212)
			pcolor(taxis,log2(faxis/500),sum(sum(STRF2,4),3)/sqrt(PP)/double(T));   %Edited Chen Oct. 2008
			shading flat,colormap jet,colorbar
	...
	if ~( T==0 | isempty(T) )
		Wo1=No1/(double(T)/NBoot);              %Edited Chen Oct. 2008
		Wo2=No2/(double(T)/NBoot);              %Edited Chen Oct. 2008
	else
	...
	if UF~=1
		for k=1:UF
			for l=1:NBoot
			dT=taxis(3)-taxis(2);
			STRF1i(:,:,k,l)=interp1(taxis,STRF1(:,:,k,l)',(-N1*UF:N2*UF-1)*dT/UF+k*dT/UF,'PCHIP')';
			STRF2i(:,:,k,l)=interp1(taxis,STRF2(:,:,k,l)',(-N1*UF:N2*UF-1)*dT/UF-k*dT/UF+dT,'PCHIP')';
			end
		end
	end
