%function [UberSTRF] = calculate_strf_params(monkeyName,sessionDate,driveID,clust2proc,custTag)
	...
	%for m = 1:numClust
		...
		try
			...
			fprintf('\n!!!Error occurred while calculating STRF params for clsuter #%i!!!\n',clust2proc(m))
		end
...
%		%h_gabor = figure;
%		%h_mod = figure;
%		numRow = floor(sqrt(numClust));
%		numCol = ceil(sqrt(numClust));
%		if numRow*numCol < numClust
...
%	end
%	if ifSave == 'y'
%		var2save = {'monkeyName','sessionDate','driveID','UberSTRF'};
%		save(sprintf('~/STRF/STRFParams/STRFParams_%s_%s_%s_%s.mat',monkeyName,sessionDate,driveID,custTag),var2save{:});
%	end
